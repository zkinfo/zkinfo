...
function xmlQuery($params, $root = 'data')
{
	$xml = toXml($params, $root);
	$xml = trim($xml);
	$xml = substr($xml, strpos($xml, "<$root"));
	return $xml;
}

function jsonQuery($params, $root = 'data')
{
	$json = array($root => (object) $params);
	$json = json_encode($json);
	return $json;
}

function tag_merge($tag1, $tag2, $return_array = false)
{
	if ( ! is_array($tag1)) $tag1 = explode(',', preg_replace('/\s/', '', $tag1));
	if ( ! is_array($tag2)) $tag2 = explode(',', preg_replace('/\s/', '', $tag2));

	$tag1 = array_keys(array_merge(array_flip($tag1), array_flip($tag2)));
...
function tag_diff($tag1, $tag2, $return_array = false)
{
	if ( ! is_array($tag1)) $tag1 = explode(',', preg_replace('/\s/', '', $tag1));
	if ( ! is_array($tag2)) $tag2 = explode(',', preg_replace('/\s/', '', $tag2));

	$tag1 = array_diff($tag1, $tag2);
