...
class MY_Loader extends MX_Loader 
{
	...
	public function model($model, $object_name = NULL, $connect = FALSE) 
	{
		...
		if ($this->config->item('javaAPI_model') && in_array(strtolower($model), $this->config->item('javaAPI_model'))) {
			$model = 'v2/' . $model;
		}
	...
	public function memcache() 
	{
		...
		CI::$APP->$_alias = new Memcache;

		$config = $this->load->config('memcached');
		$item = $this->config->item('default');

		@CI::$APP->$_alias->connect($item['host'], $item['port']);
