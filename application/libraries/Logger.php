...
class Logger
{
	...
	function log($level, $message, $filename='application')
	{
		...
		$ci = get_instance();
		if ($ci->config->item('machine_name')) {
			$filename = $ci->config->item('machine_name') . '-' . $filename;
		}
	...
	function logOne($level, $message, $filename='application')
	{
		require_once 'Zend/Log.php';
		...
		$logger = Zend_Log::factory(array(
		    ...
		    array(
		        'writerName'   => 'Stream',
		        'writerParams' => array(
		            'stream'   => APPPATH.'/logs/'.$filename.'.log',
		        ),
		        'formatterName' => 'Simple',
		        'formatterParams' => array(
		            'format'   => '%timestamp%: %message%'.PHP_EOL,
	...
	function login_log($message)
	{
		...
		$ci = get_instance();
		if ($ci->config->item('machine_name')) {
			$uid = $ci->config->item('machine_name') . '-' . $uid;
		}
	...
	public function write($level,$message,$path){
		...
		$machine_name = '';
		if ($ci->config->item('machine_name')) {
			$machine_name = $ci->config->item('machine_name') . '-';
		}
		...
		$stream =  APPPATH.'logs/'.$level."/".$machine_name.$path.'-'.date('Y-m-d').'.log';
