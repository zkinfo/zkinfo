...
class AlipayNotify {
	...
	function verifyNotify(){
		if(empty($_POST)) {//判断POST来的数组是否为空
			...
			$responseTxt = 'true';
			if (! empty($_POST["notify_id"])) {$responseTxt = $this->getResponse(rawurlencode($_POST["notify_id"]));}
	...
	function verifyReturn(){
		if(empty($_GET)) {//判断POST来的数组是否为空
			...
			//获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
			$responseTxt = 'true';
			if (! empty($_GET["notify_id"])) {$responseTxt = $this->getResponse(rawurlencode($_GET["notify_id"]));}
