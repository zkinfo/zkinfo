...
class Template {
	...
	function parse_template($tpl='') {
		...
		if ( ! is_dir(APPPATH.'./views/tpl_cache')) {
			mkdir(APPPATH.'./views/tpl_cache');
		}
		...
		//处理子页面中的代码
		$template = preg_replace("/\<\!\-\-\{template\s+([a-z0-9_\/]+)\}\-\-\>/ie", "\$this->readtemplate('\\1')", $template);		
		$template =  preg_replace("/\<\!\-\-\{mi_tpl\((.+?)\)\}\-\-\>/ie", "\$this->mitpltags('\\1')", $template);
		//处理页面中的控件
		...
		$template =  preg_replace("/\<\!\-\-\{form\((.+?)\)\}\-\-\>/ie", "\$this->formtags('\\1')", $template);
		// wk_item_info
		$template =  preg_replace("/\<\!\-\-\{wk_item_info\((.+?)\)\}\-\-\>/ie", "\$this->wkiteminfotags('\\1')", $template);
		//处理页面中的URL
		$template =  preg_replace("/\<\!\-\-\{resource\((.+?)\)\}\-\-\>/ie", "\$this->resourcetags('\\1')", $template);
		$template =  preg_replace("/\<\!\-\-\{mi_resource\((.+?)\)\}\-\-\>/ie", "\$this->mi_resourcetags('\\1')", $template);
		//解析模块调用
		//$template = preg_replace("/\<\!\-\-\{block\/(.+?)\}\-\-\>/ie", "\$this->blocktags('\\1')", $template);
		//money处理
		$template = preg_replace("/\<\!\-\-\{mi_money\((.+?)\)}\-\-\>/ie", "\$this->mimoneytags('\\1')", $template);
		//时间处理
		//$template = preg_replace("/\<\!\-\-\{date\((.+?)\)\}\-\-\>/ie", "\$this->datetags('\\1')", $template);
		//头像处理
		//$template = preg_replace("/\<\!\-\-\{avatar\((.+?)\)\}\-\-\>/ie", "\$this->avatartags('\\1')", $template);
		//PHP代码
	...
	function mimoneytags($money) {
		$this->_STEMPLATE['i']++;
		$search = "<!--MI_MONEY_TAG_{$this->_STEMPLATE['i']}-->";
		$this->_STEMPLATE['block_search'][$this->_STEMPLATE['i']] = $search;
		$this->_STEMPLATE['block_replace'][$this->_STEMPLATE['i']] = "<?php echo mi_money($money); ?>";
		return $search;
	}
	...
	function mitpltags($template='') {
		$this->_STEMPLATE['i']++;
		$search = "<!--MI_TPL_TAG_{$this->_STEMPLATE['i']}-->";
		$this->_STEMPLATE['block_search'][$this->_STEMPLATE['i']] = $search;
		$this->_STEMPLATE['block_replace'][$this->_STEMPLATE['i']] = "<?php echo mi_tpl($template); ?>";
		return $search;
	}
	...
	function formtags($parameter) {
		$paras = explode(',', $parameter);		
		$_type = trim($paras[0],'\'');
		$_idclass = $paras[1];
		$_vars = $paras[2];
		$_is = isset($paras[3]) ? $paras[3] : '';
		
		$this->_STEMPLATE['i']++;
		$search = "<!--FORM_TAG_{$this->_STEMPLATE['i']}-->";
		$this->_STEMPLATE['block_search'][$this->_STEMPLATE['i']] = $search;
		$this->_STEMPLATE['block_replace'][$this->_STEMPLATE['i']] = "<?php echo form('$_type', $_idclass, $_vars, $_is); ?>";
		return $search;
	}

	function wkiteminfotags($item) {
		$this->_STEMPLATE['i']++;
		$search = "<!--WK_ITEM_INFO_TAG_{$this->_STEMPLATE['i']}-->";
		$this->_STEMPLATE['block_search'][$this->_STEMPLATE['i']] = $search;

		$this->_STEMPLATE['block_replace'][$this->_STEMPLATE['i']] = "<?php echo wk_item_info($item); ?>";
	...
	function readtemplate($name) {
		...
		if(!file_exists($tplfile)) {
			$tplfile = APPPATH.'./views/'.$name.'.htm';
		}
